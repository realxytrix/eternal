...
local function add_player_esp(player)
	...
	local skeleton = { lines = {}, bone_set = nil, rig_detected = false }
	...
	local connection = runservice.RenderStepped:Connect(function()
		...
		if not objects.skeleton.rig_detected then
            ...
            if humanoid.RigType == Enum.HumanoidRigType.R6 then
				objects.skeleton.bone_set = bone_connections_r6
			else
				objects.skeleton.bone_set = bone_connections_r15
			end

            objects.skeleton.rig_detected = true

			for idx, bone_joint in pairs(objects.skeleton.bone_set) do
				...
				objects.skeleton.lines[idx] = { joint = joint, joint_outline = joint_outline, from_bone = from_bone, to_bone = to_bone }
			end
		end
		...
		if objects.skeleton.rig_detected then
			for _, bone_table in ipairs(objects.skeleton.lines) do
				local from_bone, to_bone = char:FindFirstChild(bone_table.from_bone), char:FindFirstChild(bone_table.to_bone)
